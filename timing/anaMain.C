...
#define _outAnaFormat "../data/timing/ana/timcal_%s_%s.root"
...
#define _outFileFormat "../data/timing/ana/timcal_%s.root"
#define _stepFormat "step%d"
#define _iterations 1+6


void anaMain(TString arg1 = "", TString arg2 = "",  TString arg3 = "") {
    ...
    Style_t colorArray[8]={kOrange, kBlack, kBlue, kGreen, kRed, kMagenta, kPink, kYellow-3};  
    ...
    for (int currentStep = 0; currentStep < _iterations; currentStep++) {
        ...
        cout<<endl<<"-----> currentStep: "<<currentStep<<" / "<<_iterations<<endl;
        ...
        for ( int k = 0; k < anaTimeCal::GEO.cryNo; k++) {
            for ( int kk = 0; kk < anaTimeCal::GEO.sidNo; kk++) {
                ...
                anaCalibs[currentStep].timeOffOut[k][kk] = currentAna.CAL.timeResidOut[k][kk] + currentAna.CAL.timeOff[k][kk];
                anaCalibs[currentStep].timeOffErrOut[k][kk] = currentAna.CAL.timeResidErrOut[k][kk];
        ...
        if (currentStep > 0) {
            int color = colorArray[currentStep-1];
            ...
            c_useless.cd();
            c_residMean.cd();
            currentAna.RES.residMean->Draw(Form("%sPX", currentStep > 1 ? "" : "A"));
            c_residMean.ForceUpdate();
            c_useless.cd();
            c_residRms.cd();
            currentAna.RES.residRms->Draw(Form("%sPX", currentStep > 1 ? "" : "A"));
