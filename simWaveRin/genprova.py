...
def get1wave(): #one event per window
    ...
    amp = ROOT.gRandom.Uniform(500.0, 1000.0)
    t0 = tbase*ROOT.gRandom.Uniform(-50.0,50.0)
    bline = ROOT.gRandom.Uniform(-50.0,50.0)
    tw = amp*np.vectorize(splineEval)(twave + t0 + 0.5*nsam*tbase - splineShi) + bline
    wave += tw

    for _ in range(ROOT.gRandom.Poisson(25.0)):
        iamp = np.abs(ROOT.gRandom.Gaus(0.0, 50.0)) #should use bertr
